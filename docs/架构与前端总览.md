...
|------|------|------|
| `/home` | 工作台 | 快捷入口、最近回测 |
| `/ops?tab=*` | 数据运维 | 快捷同步 / 作业与定时 / 通达信本地 / 执行记录 / 高级治理 |
| `/sync`、`/jobs` | （redirect） | 分别跳转 `/ops?tab=quick`、`/ops?tab=jobs` 等 |
| `/table` | 数据表 | AG Grid 查各业务表 |
| `/indicators` | 股票指标 | K 线/指标详情 |
| `/backtest/data/*` | 准备数据 | 概览 / 标准日线（表·K 线 Tab）/ 补数 / 缺口诊断 |
| `/backtest/run` | 运行回测 | 步骤条 + master-detail、配置折叠、结果 Tab、`?runId=` 深链 |
| `/backtest/guide` | 自定义策略 | 已注册策略表（按分类） |
| `/backtest-data/*` | （redirect） | 跳转 `/backtest/data/*` |

侧栏分组：**工作台** / **数据运维** / **策略回测** / **数据浏览**（可搜索、默认折叠）；顶栏 **面包屑**（`AppBreadcrumb.vue`）。
...
| 同步与健康 | `views/DataOpsLayout.vue`、`SyncView`、`JobCenterView` | `/instock/api/sync/*` |
| 回测 | `api/backtest.ts`、`features/backtest/*` | `/instock/api/backtest/*`；策略列表含 category/params |
| 回测策略目录 | `builtins_catalog.py`、`strategy_meta.py` | 见 [plan/回测策略目录.md](./plan/回测策略目录.md) |
| 标准库 K 线 | `features/charts/CanonicalKlineChart.vue`、`api/canonical.ts` | `GET /instock/api/canonical/kline`（`adjust_type` raw/qfq，`period` daily/weekly/monthly） |
...
`/backtest/data` 负责**标准日线治理**；`/backtest/run` 负责**跑撮合回测**，职责分离。全站跳转常量见 `utils/navLinks.ts`。
