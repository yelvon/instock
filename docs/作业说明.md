...
| **说明** | 源码中还有 **`save_nph_stock_top_data`（新浪龙虎榜）**，**未挂入 `main()`**；若要用需改 `main()` 自行加入或临时写小程序调用。资金流接口失败时日志会出现 **502** 等，与 Cookie/网络有关，见 [快速开始.md](./快速开始.md) 常见问题。 |
...
| **依赖** | 必须先有指标/策略表及对应日期数据；详见 [架构说明.md](./架构说明.md) **§10（回测在本项目中的含义）**。 |
...
- [快速开始](./快速开始.md)  
- [架构说明](./架构说明.md)（流水线、`execute_daily_job` 注释现状、回测含义）  
- [部署说明](./部署说明.md)  
