...
**相关文档**：[快速开始](./快速开始.md) · [作业脚本说明](./作业说明.md) · [部署说明](./部署说明.md)
...
运行 Python 时，**工作目录应为仓库根**（含 `requirements.txt` 的那一层），以便 `sys.path` 与脚本内路径解析一致（参见 [部署说明.md](./部署说明.md)）。
...
**完整列清单（自动生成）**：见 [`数据库表结构.md`](./数据库表结构.md)；修改 `tablestructure.py` 后在 `instock` 目录执行 `PYTHONPATH=. python3 scripts/gen_database_schema_doc.py` 更新该文档。

**数据覆盖与缺失日（实施计划，仓库内维护）**：见 [`数据覆盖计划.md`](./数据覆盖计划.md)。
...
若不跑 Docker，需自行用 **cron / launchd（macOS）/ Windows 任务计划** 调用等价命令（见 [部署说明.md](./部署说明.md)）。
...
| **依赖** | Python 3.11、MySQL、TA-Lib C 库；详见 README 与 [部署说明.md](./部署说明.md)。 |
...
若你发现本文与当前分支源码不一致（例如 `execute_daily_job` 又改回「全量并行」），欢迎直接改 `docs/架构说明.md` 并提交 PR。
