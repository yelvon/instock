...
**数据域与多源治理**（回测主线）：见 [`数据域.md`](数据域.md)；配置 [`instock/core/data/registry.yaml`](../instock/core/data/registry.yaml)；入库批次表 `data_batch`（`migrations/001_data_batch.sql`）。
...
编排入口：`instock/job/execute_daily_job.py`（建议用配置替代「注释掉步骤」的长期做法，见 `运维与调度.md`）。
