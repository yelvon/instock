...
| 架构与前端总览（一页） | [../架构与前端总览.md](../架构与前端总览.md) |
| 规划与实施方案 | [`docs/plan/`](./索引.md)（含 [回测Backtrader式架构.md](./回测Backtrader式架构.md)） |
| 撮合回测（Cerebro/Strategy/Broker） | `instock/core/backtest/`（`cerebro.py`、`broker.py`、`registry.py`、`strategies/`） |
...
1. **`execute_daily_job.py`**  
   仓库中曾对部分并行步骤（指标、K 线、策略、回测等）使用注释方式关闭。长期维护建议改为**显式配置**（环境变量或 YAML），并在 `运维与调度.md` 中约定「默认流水线」与「精简流水线」，避免每人本地行为不一致。
...
- 每完成一个 `路线图.md` 阶段，在本文件「进度」小节追加一行链接到 PR 或提交说明（可选）。
